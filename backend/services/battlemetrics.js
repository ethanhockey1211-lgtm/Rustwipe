...
const PAGE_SIZE = 100;
const PAGES_TO_FETCH = 15; // 1500 servers total
const REQUEST_DELAY_MS = 600; // Respectful of rate limits
...
async function fetchPage(page) {
  const response = await axios.get(`${BM_BASE}/servers`, {
    ...
    headers: {
      ...
      'User-Agent': 'RustWipeTracker/1.0',
    },
  });
...
function parseServer(raw) {
  ...
  if (!['community', 'modded', 'official', 'vanilla'].includes(serverType)) {
...
async function fetchAndCacheServers(db) {
  const upsert = db.prepare(`
    INSERT INTO servers (
      ...
      map_name, world_size, server_type, tags, rust_last_wipe, rust_born,
      header_image, server_url, last_seen, updated_at
    ) VALUES (
      @id, @name, @players, @max_players, @rank, @status, @country, @ip, @port,
      @map_name, @world_size, @server_type, @tags, @rust_last_wipe, @rust_born,
      @header_image, @server_url, @last_seen, @updated_at
  ...
  const recordWipe  = db.prepare(`
    INSERT OR IGNORE INTO wipe_history (server_id, wipe_time, detected_at)
    VALUES (?, ?, ?)
  `);
  ...
  const upsertBatch = db.transaction((servers) => {
    ...
    for (const s of servers) {
      ...
      if (s.rust_last_wipe) {
        if (!existing) {
          // New server – record its current wipe date
  ...
  for (let page = 1; page <= PAGES_TO_FETCH; page++) {
    try {
      ...
      // Exponential backoff then stop on persistent failure
